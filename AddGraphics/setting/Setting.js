  ...
  function(
    ...
    return declare([BaseWidgetSetting, _WidgetsInTemplateMixin], {
      ...
      setConfig: function(config) {
        ...
        if (options && options.lon) {
          this.lon.set('value', options.lon);
        }
        if (options && options.lat) {
          this.lat.set('value', options.lat);
        }
        if (options && options.zoom) {
          this.zoom.set('value', options.zoom);
        }
      },
      ...
      getConfig: function() {
        ...
        options.lon = this.lon.get("value");
        options.lat = this.lat.get("value");
        options.zoom = this.zoom.get("value");
